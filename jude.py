    ...
    try:
        ...
        if 1 <= choice <= len(txt_files):
            ...
            if os.path.isfile(selected_file):
                try:
                    user_choice = input(f"{mage} o.nt. If it's a page, input n (y/Y/yes/Yes or n/N/no/No or d/D/default/Default): {r}")
...
def Initialize():
    ...
    linex()
    print(f"│{mage}     ->{r}1 {r} Manual through input          │")
    print(f"│{mage}     ->{r}2 {r} Manual through File           │")
    print(f"│{mage}     ->{r}3 {r} Automatic through option      │")
    bro()
